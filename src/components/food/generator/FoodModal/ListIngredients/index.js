...
function ListIngredientsUI({ ingredients }) {
  return (
    <ul class="list-disc list-inside text-[--color-white] font-semibold">
      {ingredients &&
        ingredients.map((i, j) => <li key={"ingredient" + j}>{i.name}</li>)}
    </ul>
  );
}
...
function ListIngredientsLogique({ UI, meal, token }) {
  ...
  useEffect(() => {
    authProvider
      .getRecipe(meal.recipe.id, token)
      .then((recipe) => {
        setRecipe(recipe);
      })
      .catch((e) => {
  ...
  return <UI ingredients={recipe.ingredients} />;
}
...
export function ListIngredients({ meal }) {
  return (
    <ProvideToken
      Component={({ token }) =>
        ListIngredientsLogique({
          token: token,
          meal: meal,
          UI: ListIngredientsUI,
        })
      }
    />
  );
